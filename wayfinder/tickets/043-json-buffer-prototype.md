...
**What the prototype returns:** the state model in words (what states exist, what each transition does), the marker's behaviour handed back to whatever [view-switcher-prototype](wayfinder/tickets/042-view-switcher-prototype.md) drew for it, and the ⌘Z answer from (4), which is the one that touches a signed-off rule. Autosave is worth an explicit sentence too: the buffer is never persisted, so a reload discards it — confirm that is what happens and that it is the right thing.
