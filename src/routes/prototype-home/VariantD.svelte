<script lang="ts">
	/**
	 * PROTOTYPE — throwaway. Variant D: the synthesis Mitchell picked.
	 * C's hero, then A's hero, then a fresh middle — a deciding-not-documenting
	 * statement with margin notes, the life of one .bcc.json in five stations,
	 * the four examples as a shuffleable pad — C's pinned attribution, and the
	 * editor dock: a sliver of the real editor under a perforation you tear
	 * through by scrolling (EditorDock owns that mechanic). Every "open the
	 * editor" affordance rides the same tear, so there is exactly one way in.
	 */
	...
	// C's chip stack: the palette legend doubling as the pitch.
	const chips = [
	...
	// files, through the same registry the editor's Examples menu uses.
	...
	/** Load the example into the real editor, then ride the tear down. Same
	 * replace path as the Examples menu; a confirm guards unexported work. */
	function openInEditor(entry: ExampleEntry) {
		if (
			canvas.unexported &&
			!window.confirm(`Opening ${entry.name} replaces this canvas, and it has unexported changes. Continue?`)
		) {
			return;
		}
	...
	/** The other door: a blank sheet, same guard, same ride down. */
	function openBlank() {
		if (
			canvas.unexported &&
			!window.confirm('Starting a blank canvas replaces this one, and it has unexported changes. Continue?')
		) {
			return;
		}
	...
	/** Scroll reveal, same determined easing as everything else. */
...
<div class="mx-auto max-w-6xl px-5 pt-6 sm:px-8">
	<header class="flex items-baseline justify-between">
		...
		<a href="/" onclick={openEditor} class="rounded-[4px] border border-line bg-sheet px-3 py-1.5 text-sm font-medium hover:bg-paper">
			Open the editor
		</a>
	</header>

	<!-- Hero 1 (from C): the wall — headline left, chip stack right. Two
	     columns from md up: ~1000px viewports fell back to the stacked
	     mobile layout and the chips floated orphaned below the fold. -->
	<section class="mt-14 grid items-start gap-12 md:grid-cols-[1.2fr_1fr] md:gap-10 lg:gap-8">
		...
		<figure class="mx-auto w-full max-w-sm md:mt-2">
			<ul class="space-y-3">
...
<!-- Hero 2 (from A): the sheet, already on the table. -->
<section class="relative mt-28 min-h-[92svh] overflow-hidden">
	<img
		src={orderSvg}
		alt="The Order Fulfillment canvas, fully rendered"
		class="pointer-events-none absolute top-8 left-1/2 w-[min(1180px,140vw)] max-w-none -translate-x-1/2 sm:w-[min(1180px,94vw)]"
		width="1440"
		height="1292"
	/>
	<!-- The plate rides the sheet's own bottom edge (height = width × 1292/1440,
	     top offset 2rem) at every width, mobile included: mostly on the sheet,
	     a modest overhang onto the paper. The shadow casts upward, over the
	     sheet it slid up from — a tight pass and a wide pass for depth. -->
	<div
		use:reveal
		class="relative z-10 mx-5 mt-[calc(125.58vw-16rem)] max-w-[80vw] border border-line bg-sheet p-6 shadow-[0_-6px_16px_rgb(26_30_32/0.18),0_-24px_56px_rgb(26_30_32/0.22)] sm:mx-auto sm:mt-[calc(min(1180px,94vw)*0.897-14rem)] sm:max-w-[min(42rem,78vw)] sm:p-8 lg:ml-[10%]"
	>
		<h2 class="text-3xl font-bold tracking-tight text-balance sm:text-4xl">
			This is a bounded context, written down.
		</h2>
		<!-- No CTA here on purpose: an exit this early skips the whole middle.
		     The plate hands the reader onward; the doors are the header, hero 1
		     and the blank-canvas card after the examples. -->
		<p class="mt-4 font-serif text-lg leading-relaxed text-ink-soft">
			...
			and the real thing is waiting at the bottom of this page.
		</p>
	</div>
</section>
...
<div class="mx-auto max-w-6xl px-5 sm:px-8">
		...
		<div class="relative mt-12">
			...
			<div use:reveal class="relative grid grid-cols-[2.5rem_minmax(0,1fr)] gap-x-4 gap-y-5 pb-14 sm:gap-x-6 lg:grid-cols-[2.5rem_minmax(0,1fr)_minmax(0,1.05fr)]">
				...
				<div class="col-start-2 max-w-xl rounded-[4px] bg-ink p-4 font-mono text-xs leading-relaxed text-paper shadow-sm lg:col-start-3 lg:row-start-1 lg:max-w-none">
					...
					<p class="mt-1 text-query">all canvases read; images current</p>
				</div>
			</div>
	...
	<!-- The examples: real canvases, opened into the real editor below. -->
...
<!-- Below the attribution: the editor itself, behind the perforation. -->
<div class="mt-20">
	<EditorDock bind:this={dock} />
</div>
