One plugin covers every site generator built on [unified](https://remark.js.org).
Two lines for Astro; two paragraphs for Docusaurus, because two of its choices
fight raw HTML and inline styles. Install this repo — there is no registry
package; `#<sha>` pins a commit:
...
`rehype-stringify` both take `allowDangerousHtml: true` — and the sheet's CSS
has to reach the page one of two ways:
...
> **field note** — Most generators discard VFile warnings, so the placeholder in
> the page is usually the whole story a reader gets.
