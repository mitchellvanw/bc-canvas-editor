...
3. **Delete the 33 dead utility declarations across the 15 body markup sites** ([stylesheet-scoping](wayfinder/tickets/071-docs-stylesheet-scoping.md) decision 9). **This is the ordering constraint:** all 15 sit in markup step 5 moves into the transformer, so a dead class not deleted first is a dead class copied into a `.ts` file where nobody finds it again. **Corrected 2026-08-17, on measurement, from "the 49 across the 18":** two of the three counts this step was written with are wrong. The 3 shell sites are not step 3's — decision 8 resolves them by moving the attribution card at step 7, where their 7 declarations become **live** and produce the map's one intended appearance change, so deleting them here would silently cancel it. And 9 of the 49 are `tracking-wide` declarations that were never dead at all; they stay in the markup and ride into the transformer with the rest of the label's classes. 33 body declarations, and the built page renders byte-for-byte identically after.
