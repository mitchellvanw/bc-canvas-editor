...
- *Amended by [artifact-design](wayfinder/tickets/007-artifact-design.md) (2026-08-07):* the HTML artifact now embeds the Canvas file and is re-importable, so **HTML-artifact export also clears "unexported changes"** and `.bcc.html` import follows the same path as Canvas-file import (version check, confirmation gate, history clearing). PNG export still never clears the flag.
