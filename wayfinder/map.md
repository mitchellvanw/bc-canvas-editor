...
- **Empty state & placeholder guidance** — how a blank canvas teaches the method (ddd-crew's helper prompts per section, trait/relationship descriptions). The visual-language decision sharpened this: per-section teaching hints exist only on the empty/unfilled state, never on a filled canvas. Waits on the editing prototype for the remaining half.
...
- **UI copy** — labels, buttons, empty-state text, tone. Register is now set by the quiet-sheet visual language (calm, documentary); the concrete copy surfaces (buttons, dialogs, hints) wait on the editing prototype. Use `writing-copy`.
