...
- [Decision: in-app reference material](wayfinder/tickets/012-reference-material.md) — one minimal **Reference** dialog (chrome text control, ⌘/, modal, Esc closes) covering the keyboard grammar in four clusters plus a ddd-crew link-out; relationship patterns taught by one-liners in their picker (strings final in the ticket); the HTML artifact carries nothing beyond legend + attribution.
- [Task: assemble the hand-off spec](wayfinder/tickets/008-assemble-spec.md) — `SPEC.md` compiled from every resolution and verified against the tickets; two sign-off seams closed (classification pickers gain **— none —**; domain-role rows lose per-role description). **Signed off 2026-08-07 — the map is complete.**
