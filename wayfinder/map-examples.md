...
- [Prototype: how does "open an example" present in the chrome?](wayfinder/tickets/021-example-chooser.md) — the Examples menu wins: a quiet chrome button after Import… whose dropdown borrows Export's frame, two-line entries; `Example opened` announcement, Replace-family gate copy, and RD's "Captured mid-workshop." flag all ratified as drafted.
- [Task: build the chooser, bundle the examples, amend the spec](wayfinder/tickets/023-ship-example-chooser.md) — shipped on `main` (`94f5279`, deploy rides that push): Examples menu in the chrome, examples bundled as raw bytes through the real parse path, 13-test byte-exact pinning incl. the artifact-leak rider, README rewritten with download links, SPEC amended (§3.5 new, §1/§6.1/§8.3/§10/§14).
- [Task: live checkpoint — the examples work on the real origin](wayfinder/tickets/024-examples-live-checkpoint.md) — **destination gate passed, map closed**: WebKit against the live origin green on every line (all four examples open clean and render, gate fires-on-dirty/silent-on-clean with Cancel/Replace correct, all three exports off Royalty Distribution with the artifact-leak sweep clean and the Canvas file byte-exact to the committed example, keyboard path per the menu grammar, claim-commit push auto-redeployed green).
