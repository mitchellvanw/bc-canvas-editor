...
- **Print pass:** minimal `@media print` with clean section breaks — printing the artifact is the PDF answer. It prints **the Sheet alone**, whichever View is on screen: a printed JSON dump is nobody's PDF. This is why the script hides inactive panels with the artifact's own class and not the `hidden` attribute — the inlined app stylesheet is Tailwind's, whose preflight hides `[hidden]` with an `!important` inside `@layer base`, and cascade layers reverse for important declarations, so no unlayered rule can raise the Sheet back up. Against a plain class the print rule wins on specificity alone.
- **Size:** the two text panels are ~12KB against a font-dominated ~435KB file.
